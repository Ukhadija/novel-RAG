...
def run_verifier(query, draft_answer,model, tokenizer, retrieved_chunks, max_new_tokens=400):
...
def answer_with_verification(query, retriever,model, tokenizer, max_retries=1, top_k=5, max_edit_reverifications=1):
    ...
    while True:
        ...
        verdict_obj = run_verifier(query, draft["answer"],model, tokenizer, draft["retrieved_chunks"])
        trace.append({
            "stage": "initial", "attempt": attempt, "top_k_used": current_top_k,
            "draft_answer": draft["answer"], "verdict": verdict_obj["verdict"],
    ...
    while (
        ...
        reverify_obj = run_verifier(query, candidate_answer,model, tokenizer, draft["retrieved_chunks"])
        trace.append({
